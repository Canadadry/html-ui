...
func TestValidateAttributeError(t *testing.T) {
	tests := map[int]struct {
		...
		1: {
			...
			exp: "column cannot have attribute 'fake' possibilities are [align bg-color border-color border-rounded border-width font-color font-family font-size height padding spacing width]",
		},
		2: {
			in: buildElWidthAttr(
				TypeElLayout,
			...
			exp: "row cannot have attribute 'fake' possibilities are [align bg-color border-color border-rounded border-width font-color font-family font-size height padding spacing width]",
		},
		3: {
			in: buildElWidthAttr(
				TypeElLayout,
			...
			exp: "el cannot have attribute 'fake' possibilities are [align bg-color border-color border-rounded border-width font-color font-family font-size height padding spacing width]",
		},
		4: {
			in: buildElWidthAttr(
				TypeElLayout,
